...
# Add parent directory to path to import the simpler NdLinear implementation
sys.path.append(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
from nd_linear import NdLinear as LowRankLinear
...
class ViT(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, pool='cls', channels=3,
        ...
        self.to_patch_embedding = nn.Sequential(
            ...
            LowRankLinear(patch_dim, dim, rank=min(patch_dim, dim) // 4),
            nn.LayerNorm(dim),
        )
        ...
        self.mlp_head = LowRankLinear(dim, num_classes, rank=min(dim, num_classes) // 4)
...
class DistillWrapper(nn.Module):
    def __init__(self, *, teacher, student, temperature=1., alpha=0.5, hard=False, mlp_layernorm=False):
        ...
        self.distill_mlp = nn.Sequential(
            ...
            LowRankLinear(dim, num_classes, rank=min(dim, num_classes) // 4)
        )
...
class Attention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0.):
        ...
        self.to_qkv = LowRankLinear(dim, inner_dim * 3, rank=min(dim, inner_dim * 3) // 4)
        self.to_out = nn.Sequential(
            LowRankLinear(inner_dim, dim, rank=min(inner_dim, dim) // 4),
            nn.Dropout(dropout)
